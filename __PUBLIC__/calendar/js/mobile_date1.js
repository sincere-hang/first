...
function datt(nian,yue,dateArr){
...
// 日历里面时间戳跟当前时间戳比较；大于等于 可点击；小于不可点击；日期默认单选
    for(var k=0;k<days;k++){
        ...
        $('.date ul li').eq(k).addClass("cursor");
                        ...
                        if(mm > yue){
                            ...
                            if(mm == yue){
                                if(dateArr[i] < dm){
                                    ...
                                    console.log("guoqi");
                                    $('.date ul li').eq(k).addClass('pass_content_date');
                                    $('.date ul li').eq(k).append(`<span class="glyphicon glyphicon-signal" style="flex: 1;color: #999;display: flex;justify-content: flex-end;align-items: flex-end;font-size: 24px;"></span>`);
                                }else if(dateArr[i] == dm){
                                    console.log("jintian");
                                    // 当天会议
                                    $('.date ul li').eq(k).addClass('content_date');
                                    $('.date ul li').eq(k).append(`<span style="flex: 1;display: flex;justify-content: space-between;align-items: center;">今天</span>`);
                                }else if(dateArr[i] > dm){
                                    // 大于今天的会议
                                    $('.date ul li').eq(k).append(`<span class="glyphicon glyphicon-signal" style="flex: 1;color: #16467b;display: flex;justify-content: flex-end;align-items: flex-end;font-size: 24px;"></span>`);
                                }
                            }
